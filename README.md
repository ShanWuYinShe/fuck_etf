...
核心思路：**脚本只负责采集数据，分析和决策交给 AI**。把 [PROMPT_full.md](PROMPT_full.md) 作为系统提示词交给 AI（含消息面规则、操作纪律；持仓以 `holdings.json` 为准、标的全集以 `etf_list.txt` 为准，每轮动态读取），AI 结合行情与消息面给出操作建议。

## 使用流程

```bash
# 1. 采集数据（需联网；etf_list.txt 中每行一个 6 位代码）
...
2. 拷贝或克隆整个项目目录。
3. 首次配置：`cp holdings.example.json holdings.json`（Windows 用 `copy` 或资源管理器复制），按实际持仓只填代码/名称/买入批次（成本/数量/买入时间），**止损/减仓/加仓/目标等处理线不写文件、由 AI 每轮按实时行情推导**；`etf_list.txt` 按需增删标的。`holdings.json` 与 `etf_list.txt` 为**入库数据源（git 管理）**，改持仓/自选只改这两个文件，可跨设备同步。
...
- `.workwork/data/` 为**最近一轮**采集结果（本机缓存，不入库）：**不保留历史数据**——某项采集失败即删除其旧文件；每轮启动自动清理已移出自选列表的标的缓存。`holdings.json`（持仓，git 管理）与 `etf_list.txt`（自选，git 管理）为入库数据源，跨设备同步。
