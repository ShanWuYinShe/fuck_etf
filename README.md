...
- 实时行情：腾讯（主）+ 新浪/东财（校验）；分时 VWAP、日K 支撑目标、**十一源快讯每轮实时采集并行合并（`news_merged.json`：国内7源直连 [金十/新浪7x24/东财/同花顺/华尔街见闻/网易7x24/新浪滚动] + 国外4源走代理 [Google新闻/Yahoo财经/BBC商业/MarketWatch]）+ 重仓股公告（`news_ann.json`）**、大盘指数、**行业板块榜（`boards_up.json`/`boards_down.json`，前 10 强/10 弱板块，用于板块走势判定，AI 分析每轮必读）**、外盘（美股/港股/期货/黄金/原油/汇率）、ETF 份额与季报重仓股。
