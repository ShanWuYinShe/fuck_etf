...
6. **今日操作**：按环境定仓位上限 → 持仓优先「高抛低吸做T压成本」（成对买卖两腿 + 价差元 + 盈亏比≥1）/ 反弹减仓 / 结构位兜底止损 → 输出 **7.1 下单清单**（代码+名称+方向+挂单价+数量+单型+止损+目标+盈亏比+触发条件）。T+1 卖出腿数量 ≤ 当日可卖份额。
7. **收盘复盘**：预判校准表（10.1，兑现/偏离/失效/到期）+ 给用户 `holdings.json` 更新建议（AI 不代写）+ 明日预案。
8. **文档维护**：分析框架/流程变动更新到本文档或 `PROMPT_full.md`；策略规则以 `PROMPT_full.md` 为准，本文件只记本机执行流程与速览。
